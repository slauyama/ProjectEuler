// Question 1
// var sum = 0, arr = [];
// for (var i = 0; i < 1000; i++) {
//     if (i % 3 === 0 || i % 5 === 0) {
//         sum += i;
//         arr.push(i);
//     }
// }

// Question 2
// arr = [1, 2];
// while (arr[arr.length - 1] + arr[arr.length - 2] < 4000000) {
//     arr.push(arr[arr.length - 1] + arr[arr.length - 2] )
// }
// arr = arr.filter(function(index) {return index % 2 == 0})
// arr.forEach(function(index) {sum += index})

// Question 3
// function isPrime(value) {
//     for (var counter = 2; counter * counter <= value; counter++) {
//         if (value % counter === 0)
//             return false;
//     }
//     return true;
// }
//                  //600851475143
//                  //300425737572 
// for (var counter = 2000000000; counter; counter--) {      
//     if (600851475143 % counter === 0 && isPrime(counter)) {
//         var answer = counter
//         break;
//     }
// }
// console.log(answer);


//Question 4
// var product = 0, str = "", largest = 0;

// for (var i = 999; i > 99; i--) {
//     for (var j = 999; j > 99; j--) {
//         product = i * j;
//         str = product.toString();
        
//         mid1 = Math.floor((str.length - 1) / 2) + 1;
//         mid2 = Math.ceil((str.length - 1) / 2);

//         part1 = str.slice(0, mid1);
//         part2 = str.slice(mid2, str.length).split("").reverse().join("");
//         if (part1 == part2) {            
//             largest = largest < product ? product : largest;
//             break;
//         }       
//     }
// }

//Question 5
// var done  = false, start = 2502;

// function cleanDivide(value, amount) {
//     for (var counter = 1; counter < amount; counter++) {
//         if (value % counter !== 0)
//             return false;
//     }
//     return true;
// }

// while (!done) {
//     if (cleanDivide(start, 20)) {
//         done = true;
//     }
//     start++;
// }

// Question 6
// var sumOfSquares = 0, sqauresOfSum = 0

// for (var counter = 1; counter < 101; counter++) {
//     sumOfSquares += Math.pow(counter,2);
//     sqauresOfSum += counter;
// }

// sqauresOfSum = Math.pow(sqauresOfSum, 2);

//Question 7
// function isPrime(value) {
//     for (var counter = 2; counter < value; counter++) {
//         if (value % counter === 0)
//             return false;
//     }
//     return true;
// }
// var arr = [];
// for (var start = 1, itr = 0; itr <= 10001; start++) {
//     if (isPrime(start)) {
//         arr.push(start)
//         itr++;
//     }
// }

//Question 8
// var str ="";
// str = str.split("");
// for (var start = 0, largest = 0; start + 13 < 1000; start++) {
//     for (var i = start, product = 1; i < start + 13; i++){
//         product *= str[i];    
//     }
//     largest = largest < product ? product : largest;
// }

//Question 9
// for (var i = 1; i < 1000 - 2; i++){
//     for (var j = i+1; j < 1000 - 1; j++){
//         for (var k = j+1; k < 1000; k++){
//             var isq = Math.pow(i, 2), jsq = Math.pow(j, 2), ksq = Math.pow(k, 2);
//             if (isq + jsq === ksq && i + j + k === 1000){
//                 var answer = i * j * k;
//             }
//         }
//     }
// }

//Question 10
// function isPrime(value) {
//     for (var counter = 2; counter * counter <= value; counter++) {
//         if (value % counter === 0)
//             return false;
//     }
//     return true;
// }

// for (var arr = [], start = 1; start < 2000000; start++) {
//     if (isPrime(start)) {
//         arr.push(start);
//     }
// }

// sum = 0
// arr.forEach(function(index){sum += index;});
// console.log(arr, sum)

// Question 11
// var arr = [
// [08, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50, 77, 91, 08],
// [49, 49, 99, 40, 17, 81, 18, 57, 60, 87, 17, 40, 98, 43, 69, 48, 04, 56, 62, 00],
// [81, 49, 31, 73, 55, 79, 14, 29, 93, 71, 40, 67, 53, 88, 30, 03, 49, 13, 36, 65],
// [52, 70, 95, 23, 04, 60, 11, 42, 69, 24, 68, 56, 01, 32, 56, 71, 37, 02, 36, 91],
// [22, 31, 16, 71, 51, 67, 63, 89, 41, 92, 36, 54, 22, 40, 40, 28, 66, 33, 13, 80],
// [24, 47, 32, 60, 99, 03, 45, 02, 44, 75, 33, 53, 78, 36, 84, 20, 35, 17, 12, 50],
// [32, 98, 81, 28, 64, 23, 67, 10, 26, 38, 40, 67, 59, 54, 70, 66, 18, 38, 64, 70],
// [67, 26, 20, 68, 02, 62, 12, 20, 95, 63, 94, 39, 63, 08, 40, 91, 66, 49, 94, 21],
// [24, 55, 58, 05, 66, 73, 99, 26, 97, 17, 78, 78, 96, 83, 14, 88, 34, 89, 63, 72],
// [21, 36, 23, 09, 75, 00, 76, 44, 20, 45, 35, 14, 00, 61, 33, 97, 34, 31, 33, 95],
// [78, 17, 53, 28, 22, 75, 31, 67, 15, 94, 03, 80, 04, 62, 16, 14, 09, 53, 56, 92],
// [16, 39, 05, 42, 96, 35, 31, 47, 55, 58, 88, 24, 00, 17, 54, 24, 36, 29, 85, 57],
// [86, 56, 00, 48, 35, 71, 89, 07, 05, 44, 44, 37, 44, 60, 21, 58, 51, 54, 17, 58],
// [19, 80, 81, 68, 05, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77, 04, 89, 55, 40],
// [04, 52, 08, 83, 97, 35, 99, 16, 07, 97, 57, 32, 16, 26, 26, 79, 33, 27, 98, 66],
// [88, 36, 68, 87, 57, 62, 20, 72, 03, 46, 33, 67, 46, 55, 12, 32, 63, 93, 53, 69],
// [04, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 08, 46, 29, 32, 40, 62, 76, 36],
// [20, 69, 36, 41, 72, 30, 23, 88, 34, 62, 99, 69, 82, 67, 59, 85, 74, 04, 36, 16],
// [20, 73, 35, 29, 78, 31, 90, 01, 74, 31, 49, 71, 48, 86, 81, 16, 23, 57, 05, 54],
// [01, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52, 01, 89, 19, 67, 48]];

// var largest = 0, product = 0, i, j;
// for (i = 0; i < 20; i++) {
//     for (j = 0; j < 20; j++) {
//         if (i < 17) {
//             product = arr[i][j] * arr[i + 1][j] * arr[i + 2][j] * arr[i + 3][j]
//             largest = largest < product ? product : largest;
//         } 
//         if (j < 17) {
//             product = arr[i][j] * arr[i][j + 1] * arr[i][j + 2] * arr[i][j + 3]
//             largest = largest < product ? product : largest;
//         }
//         if (i < 17 && j < 17) {
//             product = arr[i][j] * arr[i + 1][j + 1] * arr[i + 2][j + 2] * arr[i + 2][j + 3]
//             largest = largest < product ? product : largest;
//         }
//         if (i >= 3 && j < 17) {
//             product = arr[i][j] * arr[i - 1][j + 1] * arr[i - 2][j + 2] * arr[i - 3][j + 3]
//             largest = largest < product ? product : largest;
//         }
//          console.log(largest);  
//     }
// }

// Question 12 NOT SOLVED
// function howManyFactors(value) {
//     for (var counter = 1, returnValue = 0; counter <= value; counter++) {
//         if (value % counter === 0)
//             returnValue++;
//     }

//     return returnValue;
// }

// for (var i = 1, triNum = 1; howManyFactors(triNum) < 100; ) {
//     triNum += i;
//     i++;
// }
// console.log(triNum, howManyFactors(triNum));

// Question 20
var factorial = function(number) {
	debbuger;
	var returnValue;
	returnValue = number ? number.times(factorial(number.minus(1))) : 1;
	console.log(returnValue);	
}
var hundred = new BigNumber(100);
console.log(factorial(hundred));